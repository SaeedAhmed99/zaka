...
public function ZakatCash()
  ...
  $institutions = $this->institutions($WebmasterSettings->institutions_section_id);
            ...
            // .. end of .. Page Title, Description, Keywords
                  $Fatwes = Topic::where([['status', 1], ['webmaster_id', $WebmasterSettings->home_content13_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content13_section_id], ['expire_date', null]])->orderby('date', env("FRONTEND_TOPICS_ORDER", "desc"))->paginate(10);
              //    dd($Categories);
            // Send all to the view
    ...
    public function AchievementsPageByLang($lang)
    {
        ...
        // get User Details
            ...
            // .. end of .. Page Title, Description, Keywords
                  $Achievements = Topic::where([['status', 1], ['webmaster_id', $WebmasterSettings->home_content14_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content14_section_id], ['expire_date', null]])->where('id','<>','242')->orderby('date', env("FRONTEND_TOPICS_ORDER", "desc"))->get();
              //    dd($Categories);
  $institutions = $this->institutions($WebmasterSettings->institutions_section_id);

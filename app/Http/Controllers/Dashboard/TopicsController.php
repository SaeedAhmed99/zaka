...
//order, paginate
    ...
    public function store(Request $request, $webmasterId)
    {
     ...
     /*       require_once 'C:\inetpub\smartend\core/vendor/facebook/graph-sdk/src/Facebook/autoload.php';
            $appId         = '1451749541960418'; //Facebook App ID
            $appSecret     = 'd98505c53613630fdb1aa713bc606f30'; //Facebook App Secret
...
//$pageAccessToken = $fb->getRedirectLoginHelper()->getAccessToken();
...
$accessToken="EAAUoW8fAWuIBAObAee5jxcrZBhws0l4ZBSDhTslNZC0E11I9gzMf0auyWVLQfXqEVHEeCCLWKZBuIxE6VecoKvAkSlh4sWhNViT9vcp8tK23G9COszkcYzm631qUtgg9fGM9YQPBDnjvXcvTZCbt2hzIg0jFmsj5hKudAHp0qA9GQ4gZBvZAm9Xj9WSxrwhZAXWZBDTGoCdZBDCKHYSHAiwzT6BE7QfrX2v1gZD";
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Topic)) {
                ...
                if($request->has('multi_images') ){
                    foreach($request->multi_images as $item)
                    {
                        $file_exe = $item->getClientOriginalExtension();
                        $new_name = uniqid() . '.' . $file_exe;
                        $directory = $this->uploadPath;
                        $destienation = public_path($directory);
                        $item->move($destienation, $new_name);
                        Photo::create([
                            'topic_id' => $Topic->id,
                            'title' => '',
                            'row_no' => 1,

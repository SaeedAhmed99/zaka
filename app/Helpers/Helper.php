...
class Helper
{
    ...
    static function donation_projects()
    {
        $_Loader_Projects = \Session::get('_Loader_Projects');
        if (empty($_Loader_Projects)) {
             $WebmasterSettings = WebmasterSetting::find(1);
            $_Loader_Projects =  Topic::where([['status', 1], ['webmaster_id', $WebmasterSettings->donation_content1_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->donation_content1_section_id], ['expire_date', null]])->orderby('date', env("FRONTEND_TOPICS_ORDER", "asc"))->get();
            \Session()->put('_Loader_Projects', $_Loader_Projects);

        }
        return $_Loader_Projects->where("status", true);
    }
static function prayer_times()
    {
        $_Loader_Times = \Session::get('_Loader_Times');
        if (empty($_Loader_Times)) {
            $_Loader_Times = Http::get('http://api.aladhan.com/v1/calendarByCity?city=Gaza&country=Palestine&method=2&month=09&year=2022')->json()->toArray();
 // dd($_Loader_Times);
             
           
            \Session()->put('_Loader_Times', $_Loader_Times);

        }
        return $_Loader_Times;
    }

...
<i class="material-icons">&#xe7ef;</i>
                  ...
                  <span class="nav-icon">
                    <i class="material-icons">&#xe5c3;</i>
                        ...
                        @endif
                    @endif
                     @if(Helper::GeneralWebmasterSettings("fatwa_request_status"))
                        @if(@Auth::user()->permissionsGroup->fatwa_request_status)
                            <?php
                            $currentFolder = "fatwa_requests"; // Put folder name here
                            $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                            ?>
                            <li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
                                <a href="{{ route('fatwa_requests') }}">
<span class="nav-icon">
<i class="material-icons">&#xE896;</i>
</span>
                                    <span class="nav-text">{{ __('backend.fatwas') }}</span>
                                </a>
                            </li>
                        @endif
                    @endif
                     @if(Helper::GeneralWebmasterSettings("present_project_status"))
                        @if(@Auth::user()->permissionsGroup->present_project_status)
                            <?php
                            $currentFolder = "projects/files"; // Put folder name here
                            $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                            ?>
                            <li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
                                <a href="{{ route('presenting_projects') }}">
<span class="nav-icon">
<i class="material-icons">&#xE896;</i>
</span>
                                    <span class="nav-text">{{ __('backend.projects') }}</span>
                                </a>
                            </li>
                        @endif
                    @endif
                     @if(Helper::GeneralWebmasterSettings("zakatDeclaration_status"))
                        @if(@Auth::user()->permissionsGroup->zakatDeclaration_status)
                            <?php
                            $currentFolder = "zakatDeclaration"; // Put folder name here
                            $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                            ?>
                            <li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
                                <a href="{{ route('zakat_declaration') }}">
<span class="nav-icon">
<i class="material-icons">&#xE896;</i>
</span>
                                    <span class="nav-text">{{ __('backend.zakat_declaration') }}</span>
                                </a>
                            </li>
                        @endif
                    @endif
                    @if(Helper::GeneralWebmasterSettings("donations_status"))
                        @if(@Auth::user()->permissionsGroup->donations_status)
                            <?php
                            $currentFolder = "donations"; // Put folder name here
                            $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                            ?>
                            <li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
...
<i class="material-icons">&#xE896;</i>
                                    ...
                                    <span class="nav-text">{{ __('backend.donations') }}</span>
                                </a>
                            </li>
                        @endif
                    @endif
